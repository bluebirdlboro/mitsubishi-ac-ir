...
// ===================== Mitsubishi Heavy IR Protocol =====================
// Reverse-engineered 8-byte frame at 38 kHz:
//   B0=0xFF  B1=0x00  B2=fan_low  B3=fan_complement  B4=temp|mode  B5=0xFF-B4  B6=0x2A  B7=0xD5
//
// Fan  B2/B3:  High=FF/00  Medium=BF/40  Low=9F/60
// Mode B4[3:0]:  Cool=0x6  Dry=0x5  Fan=0x4  Heat=0x3  (off sets bit 3)
// Temp B4[7:4]:  (32 - temp) & 0xF
//
// Timing (us):  Header 5950/7475, Bit mark 508, One space 3454, Zero space 1496, Trailer 7422
...
void publishACState() {
  ...
  // Note: "auto" fan mode is not supported by the hardware IR protocol.
  // It is kept in the state for HA compatibility but maps to "low" when sent.
  ...
  if (currentScene == "power_blast") doc["preset_mode"] = "Power Cool";
  else if (currentScene == "comfort") doc["preset_mode"] = "Comfort";
  else if (currentScene == "sleep") doc["preset_mode"] = "Quiet Sleep";
  else if (currentScene == "leave") doc["preset_mode"] = "Away/Off";
...
void publishSceneState() {
  ...
  if (currentScene == "power_blast") displayName = "Power Cool";
  else if (currentScene == "comfort") displayName = "Comfort";
  else if (currentScene == "sleep")   displayName = "Quiet Sleep";
  else if (currentScene == "leave")   displayName = "Away/Off";
...
void publishDiscovery() {
  ...
  // Climate entity (Home Assistant MQTT Discovery)
  {
    JsonDocument doc;
    ...
    modes.add("fan_only");
    modes.add("dry");
    // Note: "auto" mode maps to "cool" at the IR level

    JsonArray fan_modes = doc["fan_modes"].to<JsonArray>();
    fan_modes.add("high");
    fan_modes.add("medium");
    fan_modes.add("low");
    // Note: "auto" fan mode not exposed — hardware has no true auto fan;
    // "auto" silently maps to "low" (known limitation, see README)
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // Temperature
    ...
    acTemp = (uint8_t)msg.toFloat();
    Serial.printf("[AC] Temp cmd: %d\n", acTemp);
    sendACCommand(acPower, acMode, acTemp, acFan);
  }
  // Fan speed
  ...
  // Scene preset (from HA climate card or select entity)
  ...
  // Batch set_all (used by agent/set_climate wrapper)
  else if (t == topicACCommand + "/set_all") {
    JsonDocument doc;
    DeserializationError err = deserializeJson(doc, msg);
    if (err) {
      Serial.printf("[set_all] JSON parse error: %s\n", err.c_str());
      return;
    }
    bool changed = false;
    if (!doc["mode"].isNull()) {
    ...
    if (!doc["temperature"].isNull()) {
      acTemp = (uint8_t)doc["temperature"].as<int>();
      changed = true;
    }
    ...
    if (changed) {
      Serial.printf("[set_all] mode=%s temp=%d fan=%d power=%d\n",
...
void handleIRReceive() {
  ...
  if (isMitsubishiHeavy152) {
    Serial.println("Decoded: MITSUBISHI_HEAVY_152 (library-supported)");
    ...
    if (mqttClient.connected()) {
      ...
      Serial.println("[MQTT] Decoded result published");
    }

  } else if (isMitsubishiHeavy88) {
    Serial.println("Decoded: MITSUBISHI_HEAVY_88 (library-supported)");
    ...
    // Custom 8-byte Mitsubishi protocol parser (see README for frame spec)
    bool parsed = false;
    if (results.rawlen >= 131) {
      uint8_t frame[8] = {0};
      for (int b = 0; b < 8; b++) {
        for (int bit = 0; bit < 8; bit++) {
          int idx = 4 + (b * 8 + bit) * 2;  // skip header+first mark
          if (results.rawbuf[idx] > 1250) {  // threshold: 2500us / 2tick
            frame[b] |= (1 << bit);
          }
        }
      }

      Serial.print("[IR] Extracted frame: ");
      for (int i = 0; i < 8; i++) Serial.printf("%02X ", frame[i]);
      Serial.println();
      ...
      if (headerOk && tailOk && checksumOk) {
        ...
        Serial.printf("[IR] Parsed OK: Power=%s Mode=%d Temp=%u Fan=%d\n",
    ...
    if (!parsed) {
      Serial.println("WARNING: Unknown protocol, dumping raw data:");
      Serial.println(resultToSourceCode(&results));
    }
  } else {
    Serial.printf("INFO: Protocol %s, not Mitsubishi Heavy\n",
      typeToString(results.decode_type).c_str());
    Serial.println(resultToSourceCode(&results));
  }

  // Report raw data for any protocol
  if (mqttClient.connected()) {
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("[WiFi] Connection failed"); setLed(false);
  }
}
...
void setup() {
  ...
  Serial.println("====================================");
  Serial.println(" ESP32 IR AC Controller v2.1       ");
  Serial.println("====================================");
  ...
  irsend.begin();
  Serial.printf("[IR Send] Ready, pin=%u\n", IR_LED_PIN);

  irrecv.setUnknownThreshold(kMinUnknownSize);
  irrecv.enableIRIn();
  Serial.printf("[IR Recv] Ready, pin=%u\n", IR_RECV_PIN);
  ...
  Serial.println("[BOOT] Ready. Point remote at IR receiver — decoded protocols will appear in Serial Monitor.");
}
